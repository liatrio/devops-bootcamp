...
2. Add the [build number plugin](https://www.mojohaus.org/buildnumber-maven-plugin/) to the project. Push multiple snapshot builds, and then see if you can pull down an older snapshot.
3. **`Challenge:`** [Create GitHub Action jobs](https://github.com/marketplace/actions/github-action-build-chain-cross-repo-builds) to build dependent artifacts when the artifacts they depend on are built successfully. (*on successful artifact build of joda-time, the spring-petclinic should get built as well*)
