...
```
5. Save this file to `pod-definition.yaml` and apply it to the cluster with `kubectl apply -f pod-definition.yaml`
...
## Exercise 3 - Dynammic Provsioning

So far we have seen how to provision a persistent volume statically. However this can become tedious and as a cluster administrator you would have to manually provision hundreds of these for teams of developers in an orginazation. Instead we can provision PVs dynamically, allowing storage volumes to be created on demand. To achieve this a cluster admin can define as many `StorageClass` objects as needed. 
...
3. As a cluster administrator, you need to pre-create a storage class object that developers can use. Create a StorageClass definition yaml that has the provisoner `docker.io/hostpath`, a volumeBindingMode `immediate` and a name of `standard`. Apply your StorageClass definition file to your cluster with `kubectl apply -f storage-class-definition.yaml`. 
...
4. Next, create a pvc that will request resources dynamically. Make sure to use the storage class made in the previous step. Name your pvc `pvc-jenkins`. Apply your pvc definition to your cluster with `kubectl apply -f pvc-deifinition.yaml`.
...
6. Next we will use a Kubernetes deployment to deploy our Jenkins pod. Copy the Jenkins deployment definition below and save it to a yaml file called `jenkins-deployment.yaml`. 
...
9. We need a service to expose our jenkins deployment. Copy and apply the service definition configuration. 
