...
4. Build the container using the Dockerfile within the directory. Push the image to a image repository (usually Docker Hub).

5. Update the `warden-k8s.yaml` to point to your new image and apply.

> Note:
> If you're pod is unable to pull from your image repository or is having trouble, it might be worthwhile to
> upload the image into the kind cluster and turning the `imagePullPolicy` to Never

6. Apply the `validating-webhook.yaml` file to deploy the validation configuration to the Kubernetes API server.
    ```
    cat validating-webhook.yaml | sed "s/      caBundle: .*/      caBundle: ${CA}/" | kubectl apply -f -
    ```
    
7. Test your validating webhook
Tests 1, 2, and 3 should pass.
    ```kubectl apply -f test-pods```

8. Delete the pods that passed.
...
4. Ensure your `warden` pod in the `validating` namespace is using the most recent version of your image (verify using the kubectl `describe` command). Delete and recreate your validating webhook and `warden-k8s.yaml` if necessary.
...
Tests 1, 2, 3, 4, and 5 should pass.
