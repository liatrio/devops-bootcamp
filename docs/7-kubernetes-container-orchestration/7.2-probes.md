...
2. Check the logs now. Notice there are now GET requests from the probe every 10 seconds as specified by the default value for the "periodSeconds" field, indicating that nginx is ready to receive traffic.
...
- Answer whether liveness probes wait for readiness probes to succeed.
