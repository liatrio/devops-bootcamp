...
- A organization name (yourGitHub username if you aren't part of an org)
...
6. Download and unzip the latestGitHub actions runner linux release into the `GHA` home folder
7. Create a start shell script that configures and runs theGitHub actions package
...
13. Run your docker container in detached mode and verify that it was able to connect toGitHub successfully by looking at the logs
