...
?> As for the skeleton that is being provided, it includes some basic setup on grabbing information about your dynamoDB table and S3 bucket.  The `event` object contains lots of information on whatever the event was that triggered this script and since we setup an S3 bucket as our trigger, naturally that is the information that will be in there which is where we are grabbing `bucket` and `key` from.  Now that we have the information we need to fetch our S3 bucket, the rest of the script is just grabbing the data out of it for you to process.
