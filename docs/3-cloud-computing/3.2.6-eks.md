...
In order for an Ingress to route traffic from our network load balancer to our pods we also need and Ingress Controller. Read more about [Ingresses](https://kubernetes.io/docs/concepts/services-networking/ingress/) and [Ingress Controllers](https://kubernetes.io/docs/concepts/services-networking/ingress-controllers/) 

## Exercise - Adding an Ingress and Ingress Controller to your cluster
Finally we will add an Ingress controller so that you can access this cluster without having to use port-forwarding.
1. Install an Ingress controller that works with AWS.
2. Create an ingress resource to talk to the ingress controller with the name `sock-shop-ingress` inside the sock-shop namespace.
