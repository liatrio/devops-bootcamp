...
2. Create a service of type NodePort. Folow the same guidelines as the previous service, but add a nodePort for `32000`.

3. Jenkins is now accessible without port forwarding on localhost port `32000`.
...
3. Create an ingress that uses the external ip from your LoadBalancer service as the host on port `8080`. It should use the path `/` and the jenkins service name.

3. Jenkins is now accessible outside the cluster using the host specified by the ingress on port `8080`.

3. Delete your `NAME-bc-svc` namespace when finished.
