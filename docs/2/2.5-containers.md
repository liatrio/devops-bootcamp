...
If the Docker images hello-world does not exist it downloads it, from Docker Hub in this case, and installs it in the local Docker image repository. You can use `docker images` to list the locally installed images. You will see the hello-world image in this list. You can just download or update an image without starting a container with `docker pull <IMAGE NAME>`.

Once the image is available it creates and runs a container based on the image. Images define the default command that is run when the container is started. When the command exists the container stops. You can list running and stopped containers with `docker ps -a`.
...
Discuss some possible challenges of use Containers in an enterprise projection environment.
