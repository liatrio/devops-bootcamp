...
- Use a combination of numbers, upper and lower case letters and symbols to make passwords more complicated.
...
### SSH
SSH makes use of public key authentication to allow passwordless and non-interactive access to remote hosts. To use public key authentication with SSH you must generate a pair of SSH keys, copy the public key to an SSH server and add it to the `authorized_keys` file.
