...
Ansible runs locally and does not need a centralized service to manage its state. It uses SSH to connect to remote hosts execute commands so it doesn't need an agent to be installed. This makes it lightweight and easy to start using but may suffer some performance loss compared to similar tools.
...
### Inventory
, Ansible looks in */etc/ansible/hosts* for the inventory but you can specify a different inventory file by using `-i <path>` with most commands.
...
2. This is the user that Ansible will use to connect to the host via SSH and execute commands as.
