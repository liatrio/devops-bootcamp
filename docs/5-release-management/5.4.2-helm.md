...
docs/5-release-management/5.4.2-helm.md:
  ...
  exercises:
    ...
    -
      ...
      description: Create another Helm chart using the Helm Create command and configure it to deploy a GitHub Self-Hosted Runner instance.
      estMinutes: 120
      technologies:
      - Helm
      - Kubernetes
...
In this exercise we will create another Helm chart using the [Helm Create](https://helm.sh/docs/helm/helm_create/) command and configure it to deploy a GitHub self-hosted Runner instance.
...
2. Modify the values.yaml file to reference the GitHub self-hosted Runner Docker image you created in chapter 2.

3. Use the command `helm install` to install the chart you have configured.
...
4. Use Kubectl to look at the current pods which should contain the configured GitHub self-hosted Runner pod.

5. Verify your self-hosted runner registered to GitHub.
