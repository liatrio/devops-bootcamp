...
**Goal**: Refactor into separate classes for validation, calculation, formatting, and file saving. The behavior-based tests should pass unchanged after refactoring.
...
**Goal**: Create a `DiscountStrategy` interface using abstract base classes so new discount types can be added without modifying existing code. The behavior-based tests should pass unchanged after refactoring.
...
**Goal**: Use dependency injection and Python's `Protocol` so different notification backends can be easily swapped and tested. The behavior-based tests should pass unchanged after refactoring.
