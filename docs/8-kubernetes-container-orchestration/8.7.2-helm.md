...
> You can read about proper syntax [here](https://derlin.github.io/fribourg-linux-seminar-k8s-deploy-like-a-pro/03-helmfile/). For the `releases` section, specify the values file in the next bullet point instead of passing in values inline.
...
* Finally, now that the chart is generated, all that's left is to actually deploy it. For reasons stated in the previous section, use Kustomize for this process. Create a `kustomization.yaml` that only references the inflated chart (`helm-chart.yaml`) as a resource.
...
* Note that the inflated Helm Chart does not list a namespace for all of its resources. Correct this using Kustomize to deploy all resources to the `gitea` namespace.
...
* What role does Kustomize play when using Helm Charts?
  * What is the limitation of Helm, and how does Kustomize solve it?
