...
- If any container's image isn't `mayapp:latest`, reject the pod.
...
?> Certain pods might also pass, and some that should pass might fail. Take note on which pods passed/failed and try your best to fix the problem before continuing to next step. The goal is to understand why pods would pass/fail, so if you get stuck, reach out and move on.
...
- Is there a way to test the webhook server locally without needing to stritcly be inside the kubernetes cluster?
