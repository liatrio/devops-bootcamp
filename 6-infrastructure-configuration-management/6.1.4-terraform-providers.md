...
3. This should lead you to a new page where it asks you make a new repository templated from the terraform-provider-scaffolding.
...
### Structs and types
In the provider you will have two sets of data structures to hold all your information, one is based on [Terraforms types](https://developer.hashicorp.com/terraform/plugin/framework/types) which have many additional functionalities than standard Go types you will use in the other and we need both in order for the provider to function correctly.  The structs with Go types are used to create the payloads you will be sending over to the api to process while the Terraform ones are needed in order to handle the data within your state and plans.  We have already created the Go versions for you in the API so you just need to import those to start using them but you will have to impliment the Terraform ones on your own.
...
### Exercise 4: Implementing Engineer resource and datasource
   ...
   2. Think about what the Schema should look like.  What does the data the API returns going to look like? What variables does the API need vs what 
      variables will be given to us? 
   3. Impliment all of the required CRUD operations using the tutorial as a reference.  They use methods within their client wrapper to perform the API 
      calls so here is the time where you should plan how you want to organize these for the rest of your resources if you havn't already and once again,
      the wrapper is recommended to make the code easier to read and understand.

?> For this section ignore how the datasource is implimented in the tutorial as it is an incredibly uncommon approach, your datasource should be 1:1 to another resource of the same type.  Datasources are used to get information about resources not managed by terraform, and given that they arent being managed, you will lack certian information about them such as an ID so you will need to look at the API for another way to grab the desired resources. 
...
### Exercise 5: Implimenting Dev and Ops resources
...
- What would happen if the api gets reset and you terraform have a state file?
