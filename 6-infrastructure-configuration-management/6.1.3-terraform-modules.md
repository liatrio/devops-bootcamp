...
1. Create a `variables.tf` file and add the following variables. If you already have a `variables.tf` file, add them there. Note that there are several variables that should be updated as they should be specific to your stack, variables such as `vpc_name` should be updated.
...
2. Review the configuration first. The `  source  = "terraform-aws-modules/vpc/aws"
