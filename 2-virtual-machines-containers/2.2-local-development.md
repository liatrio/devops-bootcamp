...
?> Once again if you have an m1 macbook, default vagrant will not work. As Vagrant by default uses virtualbox as its provider & virtualbox does not yet work on m1 macs (as of 23/02/2023) this presents a problem. Instead if you have an m1 mac (or any arm architecture) I reccomend using Vagrant with the docker provider. This will create a container instead of a virtual machine so reading [section 2.4](https://devops-bootcamp.liatr.io/#/2-virtual-machines-containers/2.4-containers) first might help.
