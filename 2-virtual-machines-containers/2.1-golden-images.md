...
1. You will need to install the utm project. assuming you have brew  `brew install --cask utm` should work

2. You will also need an ARM64 linux iso; ideally we want to use the centos9 iso located [here](https://mirrors.centos.org/mirrorlist?path=/9-stream/BaseOS/aarch64/iso/CentOS-Stream-9-latest-aarch64-dvd1.iso&redirect=1&protocol=https) make sure to select the ARM64 version.

3. Once you have the iso downloaded open the UTM gui.
    1. create a new vm, select virtualize
    2. when prompted for "virtualization engine", you need to give your vm the iso you downloaded. probably in the local Downloads directory.
    ![](img2/UTM_iso_selection.png)
    3. Once you have an iso selected, continue to the end (default vm hard settings will work fine).
    4. Launch your vm, go through the centos9 install. (defaults/automatic should be fine for everything)
    5. Once you asks you to reboot shut the vm down and unmount your iso before rebooting.
    ![](img2/UTM_iso_unmount.png)

4. Congradulations you have a centos9 image, you can create ssh keys manually with the same steps as below
...
1. Download the [CentOS 9](https://mirrors.centos.org/mirrorlist?path=/9-stream/BaseOS/aarch64/iso/CentOS-Stream-9-latest-aarch64-dvd1.iso&redirect=1&protocol=https) ISO for a 64-bit x86 machine.
